...
const useSpecificForm = (name, { initialValues = {}, debug = false, callback = (inputs) => console.log('form submitted', inputs) }) => {
  ...
  const onSubmit = (event) => {
    ...
    let x = (parseInt(inputs.montantSouhaite) + ((parseInt(inputs.montantSouhaite) / 100) * Number(inputs.taux))) / parseInt(inputs.moisNumber)

    Swal.fire(
      'Le montant de vos mensualités s\'élève à : ' +
      x.toFixed(2) + ' DT/mois'
    )
